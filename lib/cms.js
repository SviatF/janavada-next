...
function fallbackInternalLinks(article, candidates = [], language = 'en') {
  const haystack = plainText(`${article?.title || ''} ${article?.summary || ''} ${article?.body || ''}`).toLowerCase();
  const scored = [];

  for (const candidate of candidates) {
    if (!candidate || candidate.id === article?.id || candidate.slug === article?.slug) continue;
    if (String(candidate.language || language).toLowerCase() !== String(language).toLowerCase()) continue;

    const keyword = String(candidate.primary_seo_keyword || '').trim();
    const title = String(candidate.title || '').trim();
    const anchors = [keyword, title].filter(value => value.length >= 4 && value.length <= 90);
    const anchor = anchors.find(value => haystack.includes(value.toLowerCase()));
    if (!anchor) continue;

    const url = normalizeInternalHref(
      candidate.live_url || `/${language}/${normalizeCategory(candidate.category)}/${candidate.slug}`
    );
    if (!url) continue;

    const titleTokens = plainText(title).toLowerCase().split(/\s+/).filter(token => token.length > 3);
    const overlap = titleTokens.reduce((count, token) => count + (haystack.includes(token) ? 1 : 0), 0);
    const categoryBonus = normalizeCategory(candidate.category) === normalizeCategory(article?.category) ? 4 : 0;
    const trafficBonus = candidate.traffic_tier === 'high' ? 3 : candidate.traffic_tier === 'medium' ? 1 : 0;
    const score = overlap + categoryBonus + trafficBonus + (keyword && haystack.includes(keyword.toLowerCase()) ? 4 : 0);
...
export async function getCmsArticle(slug, language = 'en') {
  ...
  const article = normalizeCmsArticle(body.data);
  const existingLinks = normalizeInternalLinks(article.internal_links, language);

  // Older CMS stories may predate structured internal_links. Add a conservative first-party
  // fallback only when a relevant published JanaVada keyword/title already appears naturally
  // in the article text. No text is invented and no unrelated anchor is forced.
  if (existingLinks.length < 2 && article.body) {
    const params = new URLSearchParams({ language: String(language), limit: '80', offset: '0' });
    const candidatesBody = await cmsFetch(`/articles?${params.toString()}`, { revalidate: 60 });
    const candidates = Array.isArray(candidatesBody?.data) ? candidatesBody.data : [];
    const fallback = fallbackInternalLinks(article, candidates, language);
    if (fallback.length) {
      const merged = [...parseJson(article.internal_links, []), ...fallback];
      article.internal_links = merged;
      article.body = injectInternalLinks(article.body, merged, language);
    }
  }
