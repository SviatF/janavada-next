...
function articleKey(article) {
  return `${article?.language || 'en'}:${article?.slug || article?.id || ''}`;
}
...
function mergeArticles(...lists) {
  ...
  return [...map.values()].sort((a, b) => {
    const A = new Date(a.published_date || a.created_date || 0).getTime();
    const B = new Date(b.published_date || b.created_date || 0).getTime();
    return B - A;
  });
}
...
export async function getHomeArticles(lang) {
  const [legacy, cms] = await Promise.all([
    cachedLatest(80),
    getCmsArticles({ language: lang, limit: 80 }),
  ]);
  return mergeArticles(
    legacy.filter(a => languageMatch(a, lang)),
    cms.filter(a => languageMatch(a, lang))
  ).slice(0, 120);
}

export async function getTrendingArticles(lang, limit = 10) {
  const [articles, views, cmsTrending] = await Promise.all([
  ...
  const cmsViews = Object.fromEntries(
    cmsTrending.map(article => [article.id, Number(article.views_14d || 0)])
  );

  return articles
    .map(article => {
      const views14d = article._source === 'cms'
        ? Number(cmsViews[article.id] || article.views_14d || 0)
        : Number(viewsMap[article.id] || 0);
...
export const getArticle = cache(async function getArticle(slug, lang) {
  const cms = await getCmsArticle(slug, lang);
  if (cms) return cms;
...
export const getCounterpart = cache(async function getCounterpart(article, lang) {
  ...
  if (article._source === 'cms' && article.translation_group_id) {
    const cmsPaired = await getCmsCounterpart(article.translation_group_id, target);
    if (cmsPaired) return cmsPaired;
  }
  ...
  const cmsSameSlug = await getCmsArticle(article.slug, target);
  if (cmsSameSlug) return cmsSameSlug;
...
export async function getCategoryArticles(category, lang) {
  const [legacy, cms] = await Promise.all([
    cachedCategory(category, 60),
    getCmsArticles({ language: lang, category, limit: 60 }),
  ]);
  return mergeArticles(
    legacy.filter(a => languageMatch(a, lang)),
    cms.filter(a => languageMatch(a, lang))
...
export async function getSitemapArticles() {
  const [legacy, cms] = await Promise.all([
    ...
    getCmsSitemapArticles(5000),
  ]);
  return mergeArticles(legacy, cms).slice(0, 10000);
}
